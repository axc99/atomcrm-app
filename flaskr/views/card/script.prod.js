  ...
  fields.map((field, i) => {
    tableRows.push({
      ...
      name: {
        ...
        value: field.name,
        maxLength: 40,
        onChange: ({ value }) => updateField(i, _extends({}, field, { name: value }))
      },
      valueType: [{
        _com: 'Field.Select',
        onChange: ({ value }) => updateField(i, _extends({}, field, { valueType: value })),
        options: valueTypeOptions,
        value: field.valueType
      }, field.valueType == 'choice' && {
        _com: 'Field.Input',
        onChange: ({ value }) => updateField(i, _extends({}, field, { choiceOptions: value })),
        multiline: true,
        maxLength: 500,
      ...
      boardVisibility: {
        _com: 'Field.Select',
        onChange: ({ value }) => updateField(i, _extends({}, field, { boardVisibility: value })),
...
view.render = () => {
  ...
  const loadFields = () => {
    ...
    app.sendReq('getFields', {}).then(result => {
  ...
  return {
    ...
    scheme: [{
      ...
      onFinish: ({ values }) => {
        setReqLoading(true);
        app.sendReq('updateCardSettings', {
