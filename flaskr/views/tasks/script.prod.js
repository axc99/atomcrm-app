...
const TaskModal = ({ opened = false, type, closeModal, task, loadTasks }) => {
  ...
  return {
    ...
    content: [{
      ...
      onFinish: ({ values }) => {
        setReqLoading(true);
        app.sendReq(type === 'create' ? 'createTask' : 'updateTask', {
          id: task && task.id,
          name: values.name,
          tasks: subtasks
        }).then(result => {
      ...
      fields: [{
        ...
        maxLength: 30,
        rules: [{ max: 30, message: strs['taskModal_form_name_length'] }, { required: true, message: strs['taskModal_form_name_required'] }]
      }, {
        '_com': 'Field.Custom',
        'label': strs['taskModal_form_subtasks'],
        'content': TaskModalSubtasks({
          subtasks,
          setSubtasks
        })
      }],
...
const DeleteTaskModal = ({ id, opened, closeDeleteModal, loadTasks, tasks }) => {
  ...
  return {
    ...
    buttons: [{
      ...
      onClick: () => {
        setReqLoading(false);
        app.sendReq('deleteTask', { id }).then(result => {
          setReqLoading(false);
...
view.render = () => {
  ...
  const closeModal = () => {
    setTaskModal(_extends({}, taskModal, {
      opened: false
    }));
  };
  ...
  const closeDeleteModal = () => {
    setDeleteTaskModal(_extends({}, deleteTaskModal, {
      opened: false
    }));
  };
  ...
  const loadTasks = () => {
    ...
    app.sendReq('getTasks', {}).then(result => {
  ...
  return {
    ...
    scheme: [TasksList(_extends({}, listData, {
      openModal,
      onDragTask,
      deleteTask
    })), TaskModal(_extends({}, taskModal, {
      loadTasks,
      closeModal
    })), DeleteTaskModal(_extends({}, deleteTaskModal, {
      closeDeleteModal,
      loadTasks,
      tasks: listData.tasks
    }))]
